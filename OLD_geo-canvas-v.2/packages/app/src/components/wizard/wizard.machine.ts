...
export type WizardMachineState = Pick<WizardState, 'step' | 'category' | 'intent' | 'transactionType' | 'employmentType' | 'availability' | 'file' | 'associatedLayerId' | 'locationMethod'>;
...
export type WizardMachineEvent =
    ...
    | { type: 'SELECT_LOCATION_METHOD'; payload: 'file' | 'draw' }
    ...
    | { type: 'ASSOCIATE_LAYER'; payload: { layerId: string } }
    ...
    switch (state.step) {
        ...
        case 'availability':
            if (event.type === 'SELECT_AVAILABILITY') {
                if (event.payload === 'now' && state.category === 'property' && state.intent === 'offer') {
                    return { ...state, step: 'locationMethod', availability: event.payload };
                }
        ...
        case 'locationMethod':
            if (event.type === 'SELECT_LOCATION_METHOD') return { ...state, step: 'location', locationMethod: event.payload };
            if (event.type === 'BACK') return { ...state, step: 'availability', locationMethod: null };
            break;
        ...
        case 'location':
            ...
            // Auto-advance to details after drawing is complete, regardless of how it was initiated.
            if (event.type === 'FINISH_DRAWING') return { ...state, step: 'details', associatedLayerId: event.payload.layerId };
            if (event.type === 'ASSOCIATE_LAYER') return { ...state, associatedLayerId: event.payload.layerId };
            if (event.type === 'FINISH_POSITIONING') return { ...state, step: 'details' };
            if (event.type === 'BACK') {
                if (state.locationMethod) {
                    return { ...state, step: 'locationMethod', locationMethod: null, file: null, associatedLayerId: null };
                }
