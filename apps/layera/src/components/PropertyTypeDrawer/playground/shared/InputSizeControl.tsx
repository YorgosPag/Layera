  ...
  return (
    <Box className={`layera-card layera-padding--lg ${className}`} data-variant="primary">
      ...
      <Box className="layera-flex layera-flex--wrap-wrap layera-flex--gap-md layera-margin-bottom--md layera-flex--justify-center">
        {inputSizeOptions.map((option, index) => (
          <Button
            key={option.value}
            variant={inputSize === option.value ? 'primary' : 'outline'}
            size={buttonState?.size || 'sm'}
            onClick={() => handleSizeChange(option.value)}
            className={`${index < inputSizeOptions.length - 1 ? 'layera-margin-right--sm' : ''} layera-button layera-button--${buttonState?.size || 'sm'} layera-button--${inputSize === option.value ? 'primary' : 'outline'}`}
          >
