                        ...
                        >
                            <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center space-x-2">
                                    <button 
                                        onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); layersActions.startEditing(layer); }}
                                    ...
                                    <button 
                                        onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); layersActions.zoomToLayer(layer.id); }}
                                        title="Zoom to layer" 
                                        className="text-xl hover:text-blue-600">🔎</button>
                                    <button 
                                        onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); layersActions.toggleVisibility(layer.id); }}
                                        title={layer.isVisible ? "Hide" : "Show"} 
                                        className="text-xl hover:text-blue-600">{layer.isVisible ? '👁️' : '🙈'}</button>
                                    <button 
                                        onClick={() => {
                                            if (window.confirm('Είστε σίγουροι ότι θέλετε να διαγράψετε αυτό το επίπεδο;')) {
                                                layersActions.removeLayer(layer.id);
                                            }
                                        }}
