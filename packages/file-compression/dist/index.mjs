...
function useFileCompression(options = {}) {
  ...
  const reportProgress = useCallback((fileId, stage, progress2, message) => {
    const progressData = {
      fileId,
      stage,
      progress: progress2,
  ...
  const compressFile = useCallback(async (file, compressionOptions) => {
    if (!engineRef.current) {
      throw new CompressionError(
  ...
  const compressFiles = useCallback(async (files, batchOptions) => {
    if (!engineRef.current) {
      throw new CompressionError(
        "Compression engine not initialized",
        "ENGINE_NOT_INITIALIZED"
  ...
  const getRecommendations = useCallback(async (file) => {
    return recommendOptimizations(file);
  }, []);
  const estimateCompression = useCallback(async (file, options2) => {
