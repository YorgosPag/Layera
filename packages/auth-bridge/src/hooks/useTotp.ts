...
interface TotpState {
  ...
  /** TOTP setup data */
  readonly setupData: TotpSetupResult | null;
  /** Εάν είναι ενεργοποιημένο το TOTP */
...
export function useTotp(
   ...
   * Ενημερώνει το state
   */
  const updateState = useCallback((updates: Partial<TotpState>) => {
   ...
   */
  const startSetup = useCallback(async (): Promise<AuthResult<TotpSetupResult>> => {
    if (!user) {
      return {
        success: false,
   ...
   * Απενεργοποιεί TOTP
   */
  const disable = useCallback(async (): Promise<AuthResult> => {
    if (!user) {
      return {
        success: false,
        error: 'Χρήστης δεν είναι συνδεδεμένος'
   ...
   */
  const regenerateBackupCodes = useCallback(async (): Promise<AuthResult<string[]>> => {
    if (!user) {
      return {
        success: false,
        error: 'Χρήστης δεν είναι συνδεδεμένος'
