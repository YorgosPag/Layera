...
var FirestoreCache = class {
  ...
  async set(key, value, ttl) {
    try {
      ...
      const cacheData = {
        ...
        expiresAt: expiresAt ? Timestamp2.fromDate(expiresAt) : null,
        hits: 0,
        lastAccess: serverTimestamp2()
      };
  ...
  async clear() {
    try {
      ...
      const snapshot = await getDocs2(coll);
  ...
  async cleanup() {
    try {
      ...
      const expiredQuery = query2(
        coll,
        where("expiresAt", "<=", now)
      );
      const snapshot = await getDocs2(expiredQuery);
      const deletePromises = snapshot.docs.map((doc2) => deleteDoc(doc2.ref));
      await Promise.all(deletePromises);
