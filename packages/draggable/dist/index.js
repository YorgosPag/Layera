...
__export(index_exports, {
  ...
  DraggableFAB: () => DraggableFAB,
  DraggableFABDefault: () => DraggableFAB_default,
  FAB_POSITIONS: () => FAB_POSITIONS,
  PACKAGE_NAME: () => PACKAGE_NAME,
  PACKAGE_VERSION: () => PACKAGE_VERSION,
  createBounds: () => createBounds,
  createDraggableConfig: () => createDraggableConfig,
  createRightBottomBounds: () => createRightBottomBounds,
  useDraggable: () => useDraggable,
  useDraggableRightBottom: () => useDraggableRightBottom
});
...
function useDraggable(initialPosition = { x: 0, y: 0 }, config = {}, eventHandlers = {}) {
  ...
  const dragStartRef = (0, import_react.useRef)({ x: 0, y: 0 });
  ...
  const constrainPosition = (0, import_react.useCallback)((newPosition) => {
    let { x, y } = newPosition;
    if (bounds) {
  ...
  const createEventData = (0, import_react.useCallback)((currentPos) => ({
  ...
  const resetPosition = (0, import_react.useCallback)(() => {
    setPosition(initialPosition);
    setIsDragging(false);
  }, [initialPosition]);
  const setConstrainedPosition = (0, import_react.useCallback)((newPosition) => {
...
function useDraggableRightBottom(initialPosition = { right: 15, bottom: 15 }, config = {}, eventHandlers = {}) {
  ...
  const [position, setPosition] = (0, import_react.useState)(initialPosition);
  const startPosRef = (0, import_react.useRef)(initialPosition);
  const startPtRef = (0, import_react.useRef)(null);
  const draggingRef = (0, import_react.useRef)(false);
  const rafRef = (0, import_react.useRef)(null);
  const { onDragStart, onDrag, onDragEnd } = eventHandlers;
  const clamp = (0, import_react.useCallback)((right, bottom) => {
