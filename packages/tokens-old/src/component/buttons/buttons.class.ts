...
`;
  ...
  // Helper για dynamic color CSS generation (σαν το cards pattern)
  static generateDynamicColorCSS() {
    let css = '';

    // Dynamic button color overrides - Same pattern as cards
    const buttonVariants = ['primary', 'secondary', 'success', 'warning', 'danger', 'info'];

    buttonVariants.forEach(variant => {
      css += `
/* Dynamic ${variant} button styling */
html[data-layera-button-${variant}="active"] .layera-button[data-variant="${variant}"] {
  background-color: var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) !important;
  border-color: var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) !important;
  color: var(--layera-color-text-on-primary) !important;
}

html[data-layera-button-${variant}="active"] .layera-button[data-variant="${variant}"]:hover:not(:disabled) {
  background-color: color-mix(in srgb, var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) 90%, black) !important;
  border-color: color-mix(in srgb, var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) 90%, black) !important;
}

html[data-layera-button-${variant}="active"] .layera-button[data-variant="${variant}"]:active:not(:disabled) {
  background-color: color-mix(in srgb, var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) 85%, black) !important;
  border-color: color-mix(in srgb, var(--layera-live-button-${variant}, var(--layera-color-primary-${variant})) 85%, black) !important;
}
`;
    });
