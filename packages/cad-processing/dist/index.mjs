...
function useCADProcessing(options = {}) {
  ...
  const dxfParserRef = useRef(null);
  const cadRendererRef = useRef(null);
  const {
    defaultOptions = {},
  ...
  const reportProgress = useCallback((stage, progress2, message, currentEntity, entitiesProcessed, totalEntities) => {
    const progressData = {
      stage,
      progress: progress2,
      message,
  ...
  const processCADFile = useCallback(async (file, processingOptions) => {
  ...
  const renderCAD = useCallback(async (cadData, renderOptions) => {
  ...
  const exportCAD = useCallback(async (cadData, exportOptions) => {
    try {
      setIsProcessing(true);
  ...
  const estimateProcessingTime = useCallback(async (file) => {
