...
const FORMAT_COMPATIBILITY: Partial<Record<SupportedFormat, {
  canTransformTo: SupportedFormat[];
  dataLossRisk: Partial<Record<SupportedFormat, 'none' | 'low' | 'medium' | 'high'>>;
  limitations: Partial<Record<SupportedFormat, string[]>>;
}>> = {
  'geojson': {
