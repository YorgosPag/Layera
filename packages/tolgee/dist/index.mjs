...
var MinimalTolgeeProvider = ({
  ...
  return /* @__PURE__ */ jsx(MinimalTolgeeContext.Provider, { value: contextValue, children });
};
...
function useDynamicTranslation() {
  ...
  const tDynamic = useCallback((key, params) => {
    return t(key, params);
  }, [t]);
  return tDynamic;
}
function usePluralTranslation() {
  const { t } = useMinimalTolgee();
  const tPlural = useCallback((key, count) => {
    return t(key, { count });
  }, [t]);
  return tPlural;
}
  ...
  return /* @__PURE__ */ jsx2(
    "select",
    {
      ...
      className: `
        ...
        ${className || ""}
      `,
      children: Object.entries(LANGUAGES).map(([code, label]) => /* @__PURE__ */ jsx2("option", { value: code, children: label }, code))
    }
  );
};

// src/index.ts
import { T, useTranslate as useTranslate2 } from "@tolgee/react";
...
function useLayeraTranslation2() {
  ...
  const changeLanguage = useCallback2(async (newLang) => {
    localStorage.setItem("i18nextLng", newLang);
    localStorage.setItem("tolgee_language", newLang);
...
export {
  LANGUAGES,
  LanguageSwitcher,
  MinimalTolgeeProvider as LayeraI18nProvider,
  MinimalTolgeeProvider as SimpleTolgeeProvider,
  T,
  TOLGEE_CONFIG,
  MinimalTolgeeProvider as TolgeeProvider,
