...
var DrawingService = class {
  ...
  addMarker(lat, lng) {
    if (!this.map) return;
    try {
      import_leaflet.default.marker([lat, lng]).addTo(this.map);
      const area = {
  ...
  addPolygonPoint(lat, lng) {
    ...
    try {
      ...
      if (this.state.polygonPoints.length === 1) {
        ...
        this.state.currentPolygon = import_leaflet.default.polygon(this.state.polygonPoints, style).addTo(this.map);
  ...
  clearAll() {
    ...
    try {
      this.map.eachLayer((layer) => {
        if (layer instanceof import_leaflet.default.Polygon || layer instanceof import_leaflet.default.Marker) {
...
var useDrawing = (options = {}) => {
  ...
  const setDrawingMode = (0, import_react.useCallback)((mode) => {
  ...
  const deleteArea = (0, import_react.useCallback)((areaId) => {
