...
__export(index_exports, {
  ...
  calculateCircleCircumference: () => calculateCircleCircumference,
  calculateCircleDiameter: () => calculateCircleDiameter,
  calculateCircleRadius: () => calculateCircleRadius,
  calculateDetectionConfidence: () => calculateDetectionConfidence,
  ...
  clearOSMCache: () => import_geo_mapping.clearOSMCache,
  detectCircleFromThreePoints: () => detectCircleFromThreePoints,
  detectGeometry: () => detectGeometry,
  fetchAdministrativeBoundary: () => import_geo_mapping.fetchAdministrativeBoundary,
  fetchBoundaryByAddressComponent: () => import_geo_mapping.fetchBoundaryByAddressComponent,
  fetchBuildingOutlines: () => import_geo_mapping.fetchBuildingOutlines,
  formatArea: () => formatArea,
  formatBearing: () => formatBearing,
  formatCoordinatesBySystem: () => formatCoordinatesBySystem,
  formatDistance: () => formatDistance,
  geometryToMapLabels: () => geometryToMapLabels,
  getCacheSize: () => import_geo_mapping.getCacheSize,
  isBoundsCached: () => import_geo_mapping.isBoundsCached,
...
var import_geo_mapping = require("@layera/geo-mapping");
...
var useGeometrySnap = (isEnabled = true) => {
  ...
  (0, import_react2.useEffect)(() => {
    const fetchData = async () => {
      ...
      if (!isEnabled || currentZoom < (import_constants.CONFIG.geoDrawing?.minSnapZoom || 16)) {
        setOsmData(null);
      ...
      try {
        const geojson = await (0, import_geo_mapping.fetchBuildingOutlines)(map.getBounds());
        setOsmData(geojson);
      } catch (error) {
    ...
    const debouncedFetch = () => {
      ...
      timeoutRef.current = window.setTimeout(fetchData, import_constants.CONFIG.geoDrawing?.debounceMs || 500);
    };
  ...
  const snapToGeometry = (0, import_react2.useCallback)(async (point2) => {
    return point2;
  }, []);
  const toggleSnapType = (0, import_react2.useCallback)((type, enabled) => {
    console.log(`Toggle snap type ${type}: ${enabled}`);
  }, []);
  const updateTolerance = (0, import_react2.useCallback)((tolerance) => {
