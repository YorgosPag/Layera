...
export const CategoryStep: React.FC<CategoryStepProps> = ({
  ...
  const handleCardClick = useCallback((cardConfig: CardConfig) => {
    if (cardConfig.step === 'category') {
      ...
      // 🚀 ENTERPRISE: Direct navigation to intent step (handled by StepOrchestrator)
      console.log('🎯 CATEGORY: Clicked on category:', category);
      // selectCategory() ήδη κάνει goToNextStep() μέσα του
      navigation.selectCategory(category).catch(error => {
        console.error('❌ Category selection failed:', error);
      });

      // 🚀 ENTERPRISE: Legacy pipeline integration (still needed for other parts)
  ...
  return (
    <>
      ...
      {/* Next Step Cards - DISABLED: Intent step handled by StepOrchestrator */}
      {false && showNextSteps && selectedCategory && !showTransactionStep && !showAvailabilityStep && (
